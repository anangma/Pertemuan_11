...
namespace PerpustakaanAppMVC.View
{
    public partial class FrmMahasiswa : Form
    {
        ...
        private void btnPerbaiki_Click_1(object sender, EventArgs e)
        {
        ...
        {
            string keyword = txtNama.Text.Trim().ToLower(); ;

            if (!string.IsNullOrEmpty(keyword))
            {
         
                var searchResults = listOfMahasiswa
                    .Where(mhs => mhs.Npm.Contains(keyword) || mhs.Nama.ToLower().Contains(keyword) || mhs.Angkatan.Contains(keyword))
                    .ToList();
        ...
        private void DisplaySearchResults(List<Mahasiswa> searchResults)
        {
            lvwMahasiswa.Items.Clear();

            foreach (var mhs in searchResults)
            {
                var noUrut = lvwMahasiswa.Items.Count + 1;
                var item = new ListViewItem(noUrut.ToString());
                item.SubItems.Add(mhs.Npm);
